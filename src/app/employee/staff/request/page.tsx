...
export default function EmployeeRequest() {
  ...
  const getData = async () => {
    const username = await JSON.parse(localStorage.getItem("responseEmployee"))
      .username;
    const acrnoym = await JSON.parse(localStorage.getItem("responseEmployee")).acronymCompany;
  ...
  const handleUpdateRequest = async () => {
    ...
    const response = await reviewRequest(acronym, data);
    if (response.data.code == 200) {
      toast({
        variant: "default",
  ...
  return (
    <div className="flex flex-col w-full min-h-screen">
      <Header name="Xét duyệt yêu cầu" hrefInfo="/employee/info" hrefLogin="/employee-login" />
