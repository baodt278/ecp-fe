...
import {LockKeyhole, Activity, Home, FilePenLine, ReceiptText, Newspaper} from 'lucide-react';
import {useEffect, useState} from "react";

export default function MenuBar() {
    const active = localStorage.getItem("client") ? JSON.parse(localStorage.getItem("client")).active : false;
        ...
        <div className="hidden border-r bg-gray-100/40 lg:block dark:bg-gray-800/40">
            ...
            <nav className="grid items-start px-4 text-sm font-medium">
                <Link
                    ...
                    href="/client">
                    {active ? <Activity className="h-6 w-6"/> : <LockKeyhole className="h-6 w-6"/>}
                    Điện năng
                </Link>
                <Link
                    onClick={() => setIsActive(1)}
                    className={isActive === 1 ? on : off}
                    href={active ? "/client/request" : "/client"}>
                    {active ? <FilePenLine className="h-6 w-6"/> : <LockKeyhole className="h-6 w-6"/>}
                    Yêu cầu
                </Link>
                <Link
                    onClick={() => setIsActive(2)}
                    className={isActive === 2 ? on : off}
                    href={active ? "/client/bill" : "/client"}>
                    {active ? <ReceiptText className="h-6 w-6"/> : <LockKeyhole className="h-6 w-6"/>}
                    Hóa đơn
                </Link>
                <Link
